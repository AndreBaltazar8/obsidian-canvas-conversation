// from https://github.com/transitive-bullshit/chatgpt-api
// license: MIT
// modified to remove node dependencies
...
async function fetchSSE(
	options: RequestUrlParam & { onMessage: (data: string) => void }
) {
	const { onMessage, ...fetchOptions } = options;
	const res = await requestUrl(fetchOptions);
	if (res.status >= 400) {
...
export class ChatGPTAPI extends AChatGPTAPI {
					...
					try {
						...
						if (convoResponseEvent.conversation_id) {
							result.conversationId =
								convoResponseEvent.conversation_id;
						}
	...
	async sendModeration(input: string) {
		...
		const res = await requestUrl({
			...
			if (r.status !== 200) {
				const error = new types.ChatGPTError(`${r.status}`);
				error.response = r;
	...
	override async refreshSession(): Promise<string> {
		...
		try {
			...
			if (!accessToken) {
				...
				error.statusText = response?.text;
				throw error;
			}
